...
import { useUserSearch, type SearchHit } from "@/lib/queries/profiles";
...
/**
 * UserPicker is a generic, debounced user search dropdown. Pass `onSelect`
 * to be notified when the user picks a profile. Pass `excludeUserIds` to
 * filter out users that should not appear (e.g. existing collaborators).
 */
export default function UserPicker({
  ...
  const filtered = (data ?? []).filter((h) => !excluded.has(h.userId));
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 rounded-xl bg-white/70 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 text-sm text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500"

...
export default function PlanDetailPage() {
  ...
  const handleSaveItem = async (updatedFields: any) => {
    ...
    try {
      ...
      const url = isNew 
        ? `${API_BASE_URL}/api/plans/${id}/items`
        : `${API_BASE_URL}/api/plans/items/${editingItem.id}`;
      ...
      if (!response.ok) throw new Error(`Failed to ${isNew ? "create" : "update"} item`);
      
      const savedItem = isNew ? await response.json() : updatedItem;

      // Update local state
      const newPlan = { ...plan };
      if (isNew) {
         newPlan.unassigned = [...(newPlan.unassigned || []), savedItem];
      } else {
        if (!editingItem.planDayId) {
          newPlan.unassigned = newPlan.unassigned.map((i: any) => i.id === editingItem.id ? savedItem : i);
        } else {
          const dayIdx = newPlan.days.findIndex((d: any) => d.id === editingItem.planDayId);
          if (dayIdx !== -1) {
            newPlan.days[dayIdx].items = newPlan.days[dayIdx].items.map((i: any) => i.id === editingItem.id ? savedItem : i);
          }
        }
      }
      setPlan(newPlan);
  ...
  const handleDeleteItem = async (itemId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/plans/items/${itemId}`, {
        method: "DELETE",
      });
  ...
  const formatTime = (timeStr: string | null) => {
    if (!timeStr) return "";
    // Handle both "10:00:00" and "0000-01-01T10:00:00Z"
                             ...
                             return (
                               <div key={item.id}>
                                 <div 
                                   ...
                                   onDragEnd={() => setActiveDragItem(null)}
                                   onClick={() => setEditingItem(item)}
                                   className={`p-3 rounded border shadow-sm flex items-start gap-3 cursor-grab active:cursor-grabbing hover:border-primary-300 dark:hover:border-primary-500 transition-colors group ${
                                      !hasTime 
                                 ...
                                 {gap !== null && (
                                   <div className="flex flex-col items-center py-2 relative">
                                      ...
                                      <div className={`px-3 py-1 rounded-full text-[11px] font-bold shadow-sm border ${
                                        gap < 0 

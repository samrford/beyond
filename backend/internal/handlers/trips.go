...
func (h *TripsHandler) GetTrip(w http.ResponseWriter, r *http.Request) {
	...
	cRows, err := h.db.Query("SELECT id, name, location, timestamp, description, photos, journal, COALESCE(hero_photo, ''), COALESCE(side_photo_1, ''), COALESCE(side_photo_2, ''), COALESCE(side_photo_3, '') FROM checkpoints WHERE trip_id = $1 ORDER BY timestamp ASC", id)
	if err != nil {
	...
	for cRows.Next() {
		...
		var photosJSON []byte
		if err := cRows.Scan(&c.ID, &c.Name, &c.Location, &c.Timestamp, &c.Description, &photosJSON, &c.Journal, &c.HeroPhoto, &c.SidePhoto1, &c.SidePhoto2, &c.SidePhoto3); err != nil {
